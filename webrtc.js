// HTTP Portion
var http = require('http');
var fs = require('fs'); // Using the filesystem module
var httpServer = http.createServer(requestHandler);
httpServer.listen(8080);

function requestHandler(req, res) {
	// Read index.html
	fs.readFile(__dirname + '/webrtc.html', 
		// Callback function for reading
		function (err, data) {
			// if there is an error
			if (err) {
				res.writeHead(500);
				return res.end('Error loading index.html');
			}
			// Otherwise, send the data, the contents of the file
			res.writeHead(200);
			res.end(data);
  		}
  	);
}


// WebSocket Portion
// WebSockets work with the HTTP server
var io = require('socket.io').listen(httpServer);
var allnicks = new Array();
//var videoUsers = new Array();
var clients = new Array();

//an array to store the number of scores here
var scores = new Array();
var count = true;
// Register a callback function to run when we have an individual connection
// This is run for each individual user that connects
io.sockets.on('connection', 
	// We are given a websocket object in our function
	function (socket) {
	        var usernumber = null;
	        usernumber = clients.length;
		clients[usernumber] = socket.id;
		
		console.log("We have a new client: " + socket.id);
		
		//io.sockets.emit('connection',videoUsers);
		socket.on('peer_id', function(data) {
			console.log("Received: 'peer_id' " + data);

			// We can save this in the socket object if we like
			socket.peer_id = data;
			console.log("Saved: " + socket.peer_id);

			// We can loop through these if we like
			for (var i  = 0; i < io.sockets.clients().length; i++) {
				console.log("loop: " + i + " " + io.sockets.clients()[i].peer_id);
			}
			
			
			//videoUsers.push(socket);
		
			// Tell everyone my peer_id
			socket.broadcast.emit('peer_id',data);
		});
		
		socket.emit('set_id', usernumber);
		//socket.broadcast('addnewuser', usernumber);
		//socket.emit('fulluserlist', clients);
		
		socket.on('addnick', function(nick){
			socket.nick = nick;
			allnicks.push(nick);
			socket.broadcast.emit('addnewuser', nick);
			socket.emit('fulluserlist', allnicks);
		});
		
		socket.on('fulluserlist', function(allnicks){
				var html = '';
				document.getElementById('userlist').innerHTML = '';
				for (var i=0; i < allnicks.length; i++){
					document.getElementById('userlist').innerHTML += ("<div id=\"id_" + allnicks[i] + "\">" + "User joined: "+ allnicks[i] + "</div>")
				}
				allnicks.html(html);
				console.log("Showed everyone joined");
				
		});
		
		socket.on('image', function(data) {
			// Data comes in as whatever was sent, including objects
			console.log("Received: 'image' ");
			socket.broadcast.emit('image',data);
		});
		
		socket.on('count', function(data){
			var count = true;
			console.log("user countnumber is " + data.currentcountnumber + data.myindexof);
			if(scores.length < data.myindexof +1 ){
				scores.push(data.currentcountnumber);
			}
			scores[data.myindexof] = data.currentcountnumber;
			socket.broadcast.emit('scores', scores);
			console.log("the length of scores package is" + scores.length);
			//scores.push(data);
			//socket.broadcast.emit('scores', scores);
		});
		
		socket.on('disconnect', function() {
			console.log("Client has disconnected");
		});
	}
);




